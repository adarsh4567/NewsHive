...
def main():
    ...
    connector_jars = ";".join([
        ...
        "file:///opt/flink/lib/flink-python-1.18.0.jar"
    ])
    ...
    # UDF for exponential decay calculation
    @udf(result_type=DataTypes.DOUBLE())
    def exponential_decay_weight(days_diff):
        """
        Calculate exponential decay weight with 7-day half-life
        Formula: weight = 0.5^(days_diff/7)
        """
        if days_diff is None:
            return 1.0
        return math.pow(0.5, days_diff / 7.0)

    table_env.create_temporary_function("exponential_decay_weight", exponential_decay_weight)
    ...
    mongodb_sink_ddl = """
        CREATE TABLE mongo_sink_table_three (
            userid STRING,
            top_category STRING,
            top_topic STRING,
    ...
    # Long-term interest tracking with exponential decay using HOP windows for overlapping analysis
    long_term_transform_query = """
        INSERT INTO mongo_sink_table_three
        SELECT
            c.userid,
            c.top_category,
            c.top_topic,
            c.window_start,
            c.window_end,
            CONCAT('{"categories":', c.categories_json, ', "topics":', t.topics_json, ', "entities":', e.entities_json, '}') AS long_term
        FROM (
            SELECT
                userid,
                top_category,
                ...
                FROM (
                    ...
                    FROM (
                        SELECT 
                            ...
                            window_start,
                            window_end
                        FROM TABLE(TUMBLE(TABLE source_table, DESCRIPTOR(event_time), INTERVAL '1' MINUTE))
    ...
    table_result = table_env.execute_sql(long_term_transform_query)
    table_result.wait()
