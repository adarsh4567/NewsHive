...
def main():
    ...
    # Add connector JARs (updated for Flink 1.14 compatibility)
    connector_jars = ";".join([
    ...
    mongodb_sink_ddl = """
        CREATE TABLE mongo_sink_table_two (
            userid STRING,
            top_category STRING,
            top_topic STRING,
    ...
    window_transform_query = """
        INSERT INTO mongo_sink_table_two
        SELECT
            c.userid,
            c.top_category,
            c.top_topic,
            c.window_start,
            c.window_end,
            CONCAT('{"categories":', c.categories_json, ', "topics":', t.topics_json, ', "entities":', e.entities_json, '}') AS medium_term
        FROM (
            SELECT
                userid,
                top_category,
